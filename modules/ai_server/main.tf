...
resource "openstack_blockstorage_volume_v3" "ollama_data_volume" {
  name = "ollama-data-volume-${var.resource_suffix}"
...
resource "openstack_compute_instance_v2" "ai_server_instance" {
  ...
  block_device {
    uuid                  = openstack_blockstorage_volume_v3.ollama_data_volume.id
    source_type           = "volume"
    boot_index            = 1
